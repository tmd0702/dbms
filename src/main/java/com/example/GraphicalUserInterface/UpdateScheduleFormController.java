...
public class UpdateScheduleFormController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        idFieldInit();
        Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(0.2), ae -> {
            cinemaNameFieldInit();
            showDateFieldInit();
    ...
    public void startTimeFieldInit() {
        ...
        for (String startTime : startTimes) {
            String movieId = getMovieObjectIDFromComBoBox(movieTitleField.getValue());
            if (!main.getProcessorManager().getScheduleManagementProcessor().isMovieSchedulingAvailable(movieId, startTime, getScreenRoomObjectIDFromComboBox(screenRoomNameField.getValue()), getCinemaObjectIDFromComboBox(cinemaNameField.getValue()), DateTimeFormatter.ofPattern("yyyy-MM-dd").format(showDateField.getValue()))) {
    ...
    public String getCinemaObjectIDFromComboBox(Object value) {
        ...
        for (int i=0; i<cinemaNames.size();++i) {
            System.out.println(value + " " + cinemaNames.get(i));
